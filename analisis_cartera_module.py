...
def mostrar_selector_graficas():
    """Selector de gráficas con checkboxes - VERSIÓN SIMPLIFICADA (sin botones)"""
    ...
    # **ACTUALIZACIÓN AUTOMÁTICA SIN BOTONES** - SE ACTUALIZA EN TIEMPO REAL
    # Verificar si los checkboxes fueron cambiados manualmente
    if set(selecciones_actuales) != set(st.session_state.graficas_analisis_activas):
        st.session_state.graficas_analisis_activas = selecciones_actuales
    
    # Mostrar contador de gráficas activas
    st.caption(f"📊 **{len(st.session_state.graficas_analisis_activas)} gráficas activas** de {len(graficas_config)} disponibles")
    
    # Mostrar estado actual (para debug, opcional)
    # st.write(f"Estado actual: {st.session_state.graficas_analisis_activas}")
...
def crear_grafica_proyeccion_credito(datos):
    """Crea proyección de vencimientos usando los 4 rangos específicos - ORDEN: mayor a menor"""
    try:
        ...
        # ✅ NUEVAS ESTADÍSTICAS ACTUALIZADAS
        col1, col2, col3 = st.columns(3)
        ...
        with col3:
            # ✅ SOLO LOS 3 RANGOS FUTUROS
            rangos_futuros = ['Vence este mes', 'Vence próximo mes', 'Vence en 2 meses']
            total_futuro = df_proyeccion[rangos_futuros].sum().sum() / 1000000
